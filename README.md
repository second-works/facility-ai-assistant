...
このMVPでは、あえてサーバーを必要としない静的HTML / JavaScript構成を採用しています。ブラウザからそのまま動かせるため、Cloudflare Pagesや社内ポータルへ配置しやすく、短時間でデモを共有できます。データベース、常駐API、Secretsを公開デモに持ち込まずに、検索・出典表示・安全ガードという利用者に見せたい価値を小さな構成で検証できます。

静的構成は「LLMを使えない」という意味ではありません。`src/llm-adapter.mjs` にOpenAI互換Local LLM接続用アダプターを実装しており、endpointのallowlist、APIキー必須化、timeout、token数、temperature、応答形式を検証します。LLMを明示的に有効化した場合だけ接続し、未設定・失敗時はLLMが回答したように見せず、根拠付きretrieval fallbackへ戻します。

ローカルRAGドキュメントアシスタントが文書登録・サーバー側RAG・LLM接続の基盤を示すのに対し、本アプリはその基盤を施設管理の確認フローへ落とし込む、軽量で共有しやすい業務UIとして位置付けています。静的デモで業務価値と安全境界を先に確認し、必要になった段階で同じアダプターをバックエンド接続へ拡張できます。
